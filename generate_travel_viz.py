...
<head>
  ...
  <title>{plan_data["destination"]} 4日旅行可视化</title>
  <style>
    :root {{
      --cream: #fff9f1;
      --cream-deep: #f7efe2;
...
<body>
  ...
  <script>
    ...
    async function loadGoogleMapsApi(apiKey) {{
      ...
      await new Promise((resolve, reject) => {{
        ...
        window.gm_authFailure = () => {{
          ...
          finishReject(new Error("InvalidKeyMapError: Google Maps authentication failed."));
        }};

        const script = document.createElement("script");
        script.dataset.googleMapsLoader = "true";
        script.src = `https://maps.googleapis.com/maps/api/js?key=${{encodeURIComponent(apiKey)}}&libraries=places&v=weekly&callback=__initGoogleMapsForTravelGuide`;
        script.async = true;
        script.defer = true;
        script.onerror = () => {{
    ...
    async function initMap() {{
      ...
      showMessage("正在加载底图", "正在向 Google Maps 发起请求。");
